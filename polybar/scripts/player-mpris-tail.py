        ...
        if self._metadata != None:
            ...
            if len(artist):
                self.metadata['artist'] = re.sub(
                    SAFE_TAG_REGEX, """\1\1""", artist[0])
            else:
                self.metadata['artist'] = ''
            self.metadata['album'] = re.sub(
                SAFE_TAG_REGEX, """\1\1""", _getProperty(self._metadata, 'xesam:album', ''))
            self.metadata['title'] = re.sub(
                SAFE_TAG_REGEX, """\1\1""", _getProperty(self._metadata, 'xesam:title', ''))
            ...
            self.metadata['genre'] = _getProperty(
                self._metadata, 'xesam:genre', '')
            self.metadata['disc'] = _getProperty(
                self._metadata, 'xesam:discNumber', '')
            self.metadata['date'] = re.sub(SAFE_TAG_REGEX, """\1\1""", _getProperty(
                self._metadata, 'xesam:contentCreated', ''))
            self.metadata['year'] = re.sub(
                SAFE_TAG_REGEX, """\1\1""", self.metadata['date'][0:4])
            self.metadata['url'] = _getProperty(
                self._metadata, 'xesam:url', '')
...
parser.add_argument('--icon-playing', default='奈')
